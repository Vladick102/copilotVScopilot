...
class Validator:
    ...
    def validate_age(self, age: str):
        ...
        return bool(re.match(pattern, age)) and 16 <= int(age) <= 99
    ...
    def validate_living_place(self, living_place: str):
        ...
        pattern = re.compile(r'^[A-Z][a-z]{2,19}\s(st\.|av\.|prosp\.|rd\.)\s[0-9][a-z0-9]$')
    ...
    def validate_email(self, email: str):
        ...
        return bool(re.match(pattern, email)) and len(email.split('@')[0]) <= 64 and len(email.split('@')[1]) <= 255

    def validate_id(self, id: str):
        """Check id with 6 digits where one is 0"""
        pattern = re.compile(r'^\d*0\d*$')
        return bool(re.match(pattern, id)) and len(id) == 6 and id.count('0') == 1
